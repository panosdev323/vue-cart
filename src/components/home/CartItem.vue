        ...
        <div class="dropdown">
          ...
          <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton1">
            <li class="dropdown-item" v-for="(cart_prd, index) in cartProducts" :key="cart_prd.id">
              <span>
                {{ index + 1 }}) 
                {{ (isMobile ? truncatedTitle(cart_prd.title) : isMedium ? truncatedTitleLg(cart_prd.title) : cart_prd.title) }} 
                - {{ cart_prd.count }} x {{ Number(cart_prd.price).toFixed(2) }}$
              </span>
